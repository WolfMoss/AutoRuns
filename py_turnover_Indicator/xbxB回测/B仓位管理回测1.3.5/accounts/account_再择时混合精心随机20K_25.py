...
account_config = {
    ...
    # ++++ 分钟偏移功能 ++++
    # 支持任意时间开始的小时级别K线
    "hour_offset": '20m',  # 分钟偏移设置，可以自由设置时间，配置必须是kline脚本中interval的倍数。默认：0m，表示不偏移。15m，表示每个小时偏移15m下单。
    # ++++ 企业微信机器人功能 ++++
    "wechat_webhook_url": '',
...
get_kline_num = 2836  # 获取多少根K线。这里跟策略日频和小时频影响。日线策略，代表多少根日线k。小时策略，代表多少根小时k
...
strategy_pool = [
    dict(
        ...
        re_timing={'name': 'Bias', 'params': [96]}  # 可选，配置再择时策略
    ),
    dict(
        name='大学生选币策略-Bias-312',
        strategy_list=[
            {
                # 策略名称。与strategy目录中的策略文件名保持一致。
                "strategy": "Strategy_大学生",
        ...
        # 配置再择时之后，可以使用 re_timing.py 进行再择时的资金曲线模拟
        re_timing={'name': 'Bias', 'params': [312]}  # 可选，配置再择时策略
    ),
    dict(
        name='大学生选币策略-Bolling1-168',
        strategy_list=[
            {
                # 策略名称。与strategy目录中的策略文件名保持一致。
                "strategy": "Strategy_大学生",
        ...
        # 配置再择时之后，可以使用 re_timing.py 进行再择时的资金曲线模拟
        re_timing={'name': 'Bolling1', 'params': [168]}  # 可选，配置再择时策略
    ),
    dict(
        name='大学生选币策略-Bolling1-336',
        strategy_list=[
            {
                # 策略名称。与strategy目录中的策略文件名保持一致。
                "strategy": "Strategy_大学生",
        ...
        # 配置再择时之后，可以使用 re_timing.py 进行再择时的资金曲线模拟
        re_timing={'name': 'Bolling1', 'params': [336]}  # 可选，配置再择时策略
    ),
    dict(
        name='大学生选币策略-Bolling2-216',
        strategy_list=[
            {
                # 策略名称。与strategy目录中的策略文件名保持一致。
                "strategy": "Strategy_大学生",
        ...
        # 配置再择时之后，可以使用 re_timing.py 进行再择时的资金曲线模拟
        re_timing={'name': 'Bolling2', 'params': [216]}  # 可选，配置再择时策略
    ),
    dict(
