...
class StrategyConfig:
    ...
    long_select_coin_num: int | float | tuple = 0.1  # 选币数量范围，表示 10% < pct <= 20%，也可以写成 (10, 20)，表示 10 < rank <= 20
    # 空头选币数量。1 表示做空一个币; 0.1 表示做空10%的币，'long_nums'表示和多头一样多的数量
    short_select_coin_num: int | float | tuple | str = 'long_nums'  #   # 空头选项中特殊设置 `long_nums` 选项，表示空头数量和多头数量保持一致，不能用于区间。注意：多头为0的时候，不能配置'long_nums'
    # 选币范围控制，
    ...
    use_custom_func: bool = False  # 是否使用自定义函数
    ...
    def init(cls, index: int, file: DummyStrategy = None, **config):
        ...
        if file.is_abstract:
            ...
            use_custom_func = config.get('use_custom_func', False)
