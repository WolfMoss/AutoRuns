...
def calc_select_factor(df, strategy: StrategyConfig) -> pd.DataFrame:
    ...
    # 读取因子信息
    ret, mcap, alpha, decl = strategy.factor_list

    df['Ret排名'] = df.groupby('交易日期')[ret.col_name].rank(ascending=ret.is_sort_asc, method='min')
    df['市值_排名'] = df.groupby('交易日期')[mcap.col_name].rank(ascending=mcap.is_sort_asc, method='min')
    df['alpha95_排名'] = df.groupby('交易日期')[alpha.col_name].rank(ascending=alpha.is_sort_asc, method='min')

    # 计算复合因子
    df['复合因子'] = df['Ret排名'] + df['市值_排名'] + df['alpha95_排名']
