...
from config import data_path, order_path, proxy, error_webhook_url
from core.account_manager import get_usable_account_config_dict, init_config
...
first_account_exchange = default_config.bn.exchange
...
def clear_delist_pos(delist):
    # =====获取账户信息,并且筛选含有现货的账户
    account_config_dict = get_usable_account_config_dict()
    ...
    for account_name, account_config in account_config_dict.items():
        ...
        file_path = file_path / 'shift_select_coin_result.pkl'
        # 判断是否需要更新本地选币文件
        if file_path.exists():  # 文件存在，说明需要更新最近的选币文件
            # 读取本地存储的文件
