...
class StandardClient(BinanceClient):
    ...
    def transfer_u_from_spot_to_swap(self, amount, asset='USDT'):
        self.logger.info(f'从现货划转U到U本位合约: {amount} {asset}')
        if amount <= 0:
            self.logger.warning(f'当前划转金额【{amount}】小于0，本次不进行划转。(不是错误不要担心)')
            return

        if self.is_pure_long and asset == 'USDT':
            self.logger.warning('纯多策略不划转钱到合约')
            return
        ...
        self.logger.info(f'BN 资金转移 {asset} 现货账户 => U本位合约账户 接口返回: {transfer_info}')
        self.logger.ok(f'BN 资金转移 {asset} 现货账户 => U本位合约账户 资金量: {amount}')
    ...
    def transfer_u_from_swap_to_spot(self, amount, asset='USDT'):
        ...
        :param asset:       划转资产
        """
        self.logger.info(f'从U本位合约划转U到现货: {amount} {asset}')
        if amount <= 0:
            self.logger.warning(f'当前划转金额【{amount}】小于0，本次不进行划转。(不是错误不要担心)')
            return
        ...
        self.logger.info(f'BN 资金转移 {asset} U本位合约账户 => 现货账户 接口返回: {transfer_info}')
        self.logger.ok(f'BN 资金转移 {asset} U本位合约账户 => 现货账户 资金量: {amount}')
        ...
        if self.coin_margin:
            for _symbol, _coin_balance in self.coin_margin.items():
                if _symbol.replace('USDT', '') in equity['asset'].to_list():
                    ...
                    self.logger.warning(f'合约账户未找到 {_symbol} 的资产，无法计算 {_symbol} 的保证金')
    ...
    def replenish_bnb(self, buy_bnb_value, is_use_spot=True):
        ...
        if swap_usdt < buy_bnb_value:
            self.logger.warning(f'当前合约账户usdt余额【{swap_usdt:.4f}】不足以购买BNB补充手续费，跳过补充BNB操作')
            return 0
        ...
        if condition:
            ...
            if all_buy_bnb_value < 10:
                self.logger.warning('补充现货和合约BNB总价值不足10U，跳过补充BNB操作')
                all_buy_bnb_value = 0
            else:
                ...
                try:
                    ...
                    self.logger.error('划转U到现货账户报错，准备尝试消耗现货预留手续费资金...')
                    self.logger.debug(e)
