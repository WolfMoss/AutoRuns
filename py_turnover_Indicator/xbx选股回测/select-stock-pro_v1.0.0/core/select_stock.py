...
def concat_select_results(conf: BacktestConfig) -> pd.DataFrame:
    ...
    all_select_result_df_list = []  # 存储每一个策略的选股结果
    ...
    # 如果没有任何策略的选股结果，就直接返回
    if all_select_result_df_list:
        # 聚合选股结果
        all_select_result_df = pd.concat(all_select_result_df_list, ignore_index=True)
    else:
        all_select_result_df = pd.DataFrame(columns=RES_COLS)

    all_select_result_df = all_select_result_df.sort_values(by=['交易日期', '持仓周期'])[RES_COLS].rename(
        columns={'交易日期': '选股日期'}).reset_index(drop=True)
    all_select_result_df.to_pickle(select_result_path)
    # 保存一份给你核对结果用😃
    all_select_result_df.to_csv(select_result_path.with_suffix('.csv'), encoding='utf-8-sig', index=False)
