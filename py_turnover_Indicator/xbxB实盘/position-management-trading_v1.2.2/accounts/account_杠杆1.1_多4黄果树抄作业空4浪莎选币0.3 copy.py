...
# 此处只是展示配置的结构，具体配置情参考 accounts 文件夹下的 account1.py
# ====================================================================================================
strategy_name = 'account_杠杆1.1_多5黄果树抄作业空5'  # 当前账户运行策略的名称。可以自己任意取
    ...
    dict(
        ...
        strategy_list=[
            # === 1.大学生多头(资金占比0.5)
            {
                ...
                'short_cap_weight': 0,
                # 选币数量
                'long_select_coin_num': 0.3,  # 可适当减少选币数量，对策略收益影响大
            ...
            # =2.1 先使用QuoteVolumeMean，再使用动量因子，资金占比为空头的0.75(总资金的0.375)
            {
                ...
                # 选币因子信息列表，用于2_选币_单offset.py，3_计算多offset资金曲线.py共用计算资金曲线
                "factor_list": [
                    ('MinMax', False, 216, 1),  # 多空因子名（和factors文件中相同），排序方式，参数，权重。支持多空分离，多空选币因子不一样；
                ],
                "filter_list": [
                    ('QuoteVolumeMean', 216, 'pct:<0.2', False),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
                "filter_list_post": [
                    ('ZfAbsMean', 216, 'val:<0.5'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
            },
            # =2.2 先使用动量因子，再使用QuoteVolumeMean，资金占比为空头的0.25(总资金的0.125)
            {
                ...
                # 选币因子信息列表，用于2_选币_单offset.py，3_计算多offset资金曲线.py共用计算资金曲线
                "factor_list": [
                    ('QuoteVolumeMean', True, 240, 1),  # 多空因子名（和factors文件中相同），排序方式，参数，权重。支持多空分离，多空选币因子不一样；
                ],
                "filter_list": [
                    ('MinMax', 240, 'pct:<0.2'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
                "filter_list_post": [
                    ('ZfAbsMean', 240, 'val:<0.5'),  # 后置过滤filter_list_post，三种形式：pct, rank, val；支持多空分离，多空过滤因子不一样；
                ],
            },
        ]
    ),
