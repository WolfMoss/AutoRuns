...
class PortfolioMarginClient(BinanceClient):
    ...
    def set_manual_repay_futures(self):
        ...
        self.logger.ok(f'统一账户设置手动还款完成，完成结果：{repay_res}')
    ...
    def set_auto_repay_futures(self):
        ...
        self.logger.ok(f'统一账户设置自动还款完成，完成结果：{repay_res}')
    ...
    def repay_futures_negative_balance(self):
        ...
        self.logger.ok(f'统一账户清还合约负余额完成，完成结果：{repay_res}')

    def transfer_u_from_spot_to_swap(self, amount, asset='USDT'):
        self.logger.warning('统一账户暂不支持划转U到合约钱包')
    ...
    def transfer_bnb_from_margin_to_um(self, amount):
        ...
        """
        if amount <= 0:
            self.logger.info(f'当前划转金额【{amount}】小于0，本次不进行划转。(不是错误不要担心)')
            return
        ...
        self.logger.info(f'BN 资金转移BNB 杠杆账户 => U本位合约账户 接口返回: {transfer_info}')
        self.logger.info(f'BN 资金转移BNB 杠杆账户 => U本位合约账户 资金量: {amount}')
    ...
    def transfer_bnb_from_um_to_margin(self, amount):
        ...
        self.logger.info(f'BN 资金转移 BNB U本位合约账户 => 杠杆账户 接口返回: {transfer_info}')
        self.logger.info(f'BN 资金转移 BNB U本位合约账户 => 杠杆账户 资金量: {amount}')
    ...
    def replenish_bnb(self, buy_bnb_value, is_use_spot=True):
        ...
        if 'USDT' in balance['symbol'].to_list():
            ...
            if usdt < buy_bnb_value:
                self.logger.warning(f'当前账户usdt余额【{usdt:.4f}】不足以购买BNB补充手续费，跳过补充BNB操作')
                return 0
        else:
            self.logger.warning(f'当前账户未查询到usdt币种，跳过补充BNB操作')
            return 0
        ...
        self.logger.info(f'当前BNB剩余数量：现货 {spot_bnb_value:.4f} U，合约 {swap_bnb_value:.4f} U')
        ...
        if condition:
            ...
            if all_buy_bnb_value < 10:
                self.logger.info('补充现货和合约BNB总价值不足10U，跳过补充BNB操作')
                all_buy_bnb_value = 0
            else:
                self.logger.info(f'补充现货和合约BNB总价值 {all_buy_bnb_value:.4f} U，准备开始补充BNB······')
    ...
    def rebalance_bnb(self, is_use_spot):
        ...
        if is_use_spot:  # 现货模式，设置现货账户与合约账户两边BNB平衡
            ...
            if transfer_bnb > 0:  # 若大于0，表示需要向合约划转
                ...
                self.logger.info('划转BNB数量为0，跳过划转操作')
    ...
    def get_account_overview(self):
        ...
        self.logger.ok('获取账户资产数据完成')

        self.logger.info('准备处理资产数据...')
        # 追加USDT后缀，方便计算usdt价值
        ...
        negative_balance = (
                balance['杠杆钱包价值'].sum()
                + balance['crossMarginInterest'].sum()
        ...
        if self.coin_margin:
            for _symbol, _coin_balance in self.coin_margin.items():
                if _symbol in balance['symbol'].to_list():
                    ...
                    self.logger.warning(f'现货杠杆钱包未找到 {_symbol} 的资产，无法计算 {_symbol} 的保证金')
    ...
    def fetch_transfer_history(self, start_time=datetime.now()):
        ...
        for _ in add_type + reduce_type:
            ...
            try:
                ...
                self.logger.info(e)
                self.logger.info(f'当前账户查询类型【{_}】失败，不影响后续操作，请忽略')
                continue
        ...
        for transfer_type in [1, 2]:  # 1: 划入。从主账号划转进来  2: 划出。从子账号划转出去
            ...
            try:
                ...
                self.logger.info(e)
                self.logger.info(f'当前账户查询类型【{transfer_type}】失败，不影响后续操作，请忽略')
                continue
    ...
    def place_spot_order(self, symbol, side, quantity, price=None, **kwargs):
        divider(f'`{symbol}`杠杆现货下单 {side} {quantity}', '.', _logger=self.logger)
        # 确定下单参数
        params = {
            'symbol': symbol,
            'side': side,
        ...
        try:
            self.logger.info(f'杠杆现货下单参数：{params}')
            # 下单
            ...
            self.logger.ok(f'杠杆现货下单完成，杠杆现货下单信息结果：{order_res}')
        except Exception as e:
            self.logger.error(f'杠杆现货下单出错：{e}')
    ...
    def place_swap_order(self, symbol, side, quantity, price=None, reduce_only=False, **kwargs):
        ...
        """
        divider(f'`{symbol}`U本位合约下单 {side} {quantity}', '.', _logger=self.logger)
        ...
        try:
            self.logger.info(f'U本位合约下单参数：{params}')
            # 下单
            order_res = retry_wrapper(
            ...
            self.logger.ok(f'U本位合约下单完成，U本位合约下单信息结果：{order_res}')
        except Exception as e:
            self.logger.error(f'U本位合约下单出错：{e}')
            send_wechat_work_msg(
    ...
    def buy_spot(self, spot_symbol, coin_amount) -> dict:
        ...
        self.logger.debug(f'交易回执:\n{spot_order_info}')
        self.logger.ok(f'买币完成')
