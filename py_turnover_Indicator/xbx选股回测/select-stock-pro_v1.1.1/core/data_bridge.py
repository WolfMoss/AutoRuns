...
def load_dividend_delivery(file_path: str, candle_df: pd.DataFrame, save_cols: list):
    ...
    keep_cols = ['近一年分红', '分红率_登记日', '分红率_登记日_近年均值', '分红率_登记日_近年标准差',
                 '分红率_登记日_近年次数', '连续分红年份']
    ...
    if path.exists():
        ...
        # 将分红数据与日线数据和财务数据合并
        temp = pd.merge(left=candle_df[['交易日期', '收盘价']], right=dividend_data[['交易日期'] + keep_cols],
                        on=['交易日期'], how='left')
...
def _load_min_close(file_path: str, candle_df: pd.DataFrame, save_cols: list):
    # 个股股票代码
    code = candle_df['股票代码'].iloc[0]
    # 个股分钟数据路径
    path = Path(file_path) / (code + '.csv')
    if path.exists():
        min_data = pd.read_csv(path, encoding='gbk', parse_dates=['交易日期'], skiprows=1)
...
def load_15min_data(file_path: str, candle_df: pd.DataFrame, save_cols: list):
    save_cols = ['0945', '1000', '1015', '1030', '1045', '1100', '1115', '1130', '1315', '1330', '1345', '1400', '1415',
                 '1430', '1445', '1500']
    # 如果加载过5分钟的数据，就不能加载15分钟的数据
    if '0945' in candle_df.columns:
        raise Exception('疑似已经加载过【5分钟数据】，不能再加载【15分钟数据】')
    candle_df = _load_min_close(file_path, candle_df, save_cols)
    return candle_df


def load_5min_data(file_path: str, candle_df: pd.DataFrame, save_cols: list):
    save_cols = ['0935', '0940', '0945', '0950', '0955', '1000', '1005', '1010', '1015', '1020', '1025', '1030', '1035',
                 '1040', '1045', '1050', '1055', '1100', '1105', '1110', '1115', '1120', '1125', '1130', '1305', '1310',
                 '1315', '1320', '1325', '1330', '1335', '1340', '1345', '1350', '1355', '1400', '1405', '1410', '1415',
                 '1420', '1425', '1430', '1435', '1440', '1445', '1450', '1455', '1500']
    # 如果加载过15分钟的数据，就不能加载5分钟的数据
    if '0945' in candle_df.columns:
        raise Exception('疑似已经加载过【15分钟数据】，不能再加载【5分钟数据】')
    candle_df = _load_min_close(file_path, candle_df, save_cols)
    return candle_df
